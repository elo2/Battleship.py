...
def ships(num_ships,board):
    ...
    for i in range(num_ships):
        ...
        print "Legnth of ship #", i+1, "=", ship_len
...
def guess_hit_bitch():
    ...
    guess_hit.split(",")
    if (len(guess_hit.split(",")) == 2):
        x_str,y_str = guess_hit.split(",")
        try:
            guess_hit_x = int(x_str)
        except ValueError:
            print "Nice try, please enter a valid coordinate"
            return guess_hit_bitch()
        try:
...
for turn in range(20):
    guess_hit_x, guess_hit_y = guess_hit_bitch()
    guess_hit = [guess_hit_x, guess_hit_y]
